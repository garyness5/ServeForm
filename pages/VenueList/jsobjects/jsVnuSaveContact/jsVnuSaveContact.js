export default {
	async save(saveAndNew = false, bypassDuplicate = false) {
		...
		try {
			...
			const result =
				await qryVnuSaveContactMaster.run();

			const savedRow =
				Array.isArray(result)
					? result[0]
					: Array.isArray(
							qryVnuSaveContactMaster.data
						)
						? qryVnuSaveContactMaster.data[0]
						: result;
