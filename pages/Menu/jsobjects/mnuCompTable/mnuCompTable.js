export default {
	...
	async loadFromQuery() {
		const queryRows = getMnuComponents.data || [];
	...
	categoryOptions(row) {
		...
		const categories = (getMnuComponentItems.data || [])
	...
	itemOptions(row) {
		...
		return (getMnuComponentItems.data || [])
			.filter(i => i.item_type === itemType)
	...
	rowsForSave() {
		...
		return this.normalizeRows(rows)
			...
			.map((r, index) => {
				...
				const unit = (getMnuComponentUnits.data || []).find(u =>
	...
	unitOptions(row) {
		...
		return (getMnuComponentUnits.data || [])
