export default {
	...
	isProposalMode() {
		return jsPropData.hasSelectedProposal();
	},
	...
	queryRows() {
		...
		return (
			Array.isArray(
				qryEvtGetSelectedPropMenus.data
			)
			? qryEvtGetSelectedPropMenus.data
			: []
		)
	...
	menuOptions(row) {
		...
		return (qryEvtGetComponentItems.data || [])
			.filter(item =>
							item.active !== false &&
							item.deleted !== true
						 )
