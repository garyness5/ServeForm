export default {
	...
	async loadFromQuery() {
		const queryRows = getDshComponents.data || [];
	...
	categoryOptions(row) {
		...
		const categories = (getDshComponentItems.data || [])
	...
	itemOptions(row) {
		...
		return (getDshComponentItems.data || [])
			.filter(i => i.item_type === itemType)
	...
	async onItemChange(row) {
		...
		const item = (getDshComponentItems.data || []).find(i =>
	...
	rowsForSave() {
		...
		return this.normalizeRows(rows)
			...
			.map((r, index) => {
				...
				const unit = (getDshComponentUnits.data || []).find(u =>
	...
	unitOptions(row) {
		...
		return (getDshComponentUnits.data || [])
