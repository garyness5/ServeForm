export default {
	...
	async setRows(rows) {
		...
		await storeValue("dsh_components_local_rows", normalized);
		return normalized;
	},

	getRows() {
		const rows = appsmith.store.dsh_components_local_rows;
	...
	async loadFromQuery() {
		const queryRows = qryGetDshComponents.data || [];
		return await this.setRows(queryRows);
	},

	mergeUpdatedRows() {
		const rows = this.getRows();
		const updates = tblDshComponents.updatedRows || [];
	...
	categoryOptions(row) {
		...
		const categories = (qryGetDshComponentItems.data || [])
	...
	itemOptions(row) {
		...
		return (qryGetDshComponentItems.data || [])
			.filter(i => i.item_type === itemType)
	...
	async onItemChange(row) {
		...
		const item = (qryGetDshComponentItems.data || []).find(i =>
	...
	rowsForSave() {
		...
		return this.normalizeRows(rows)
			...
			.map((r, index) => {
				...
				const unit = (qryGetDshComponentUnits.data || []).find(u =>
	...
	unitOptions(row) {
		...
		return (qryGetDshComponentUnits.data || [])
	...
	totalCost() {
		...
		const extra = Number(inpDshExtraPercent.text || 0);
	...
	costPerYieldUnit() {
		...
		const yieldQty = Number(inpDshYieldQty.text || 0);
	...
	debugUpdatedRows() {
		return {
			...
			updatedRows: tblDshComponents.updatedRows || [],
			tableData: tblDshComponents.tableData || []
		};
	}
}